...
public class TeacherActivity extends AppCompatActivity
    ...
    InformationTracker join_tracker;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rubrics_menu_list = menu.addSubMenu("Available Signatures");
    ...
    private void loadActivityView() {
        ...
        */
        ...
        InformationTracker tracker = new InformationTracker(InformationTracker.STUDENTS_TRACKER_DEEP, database, username);
        tracker.addListener(this);
        this.trackers.add(tracker);

        InformationTracker tracker2 = new InformationTracker(InformationTracker.SIGNATURE_TRACKER, database, "view");
    ...
    private void fillNavBar(HashMap<String, String> hs) {
        for(String k: hs.keySet()) {
            if(!rubrics_ids.contains(k)) {
                boolean check = false;
                for(String e: courses_id_stamps.split(";")) {
                    if (e.equals(k)) {
                        check = true;
                        break;
                    }
    ...
    private void requestPetition(HashMap<String, String> hs) {
        join_tracker.unSubscribeFromSource();
        join_tracker.removeListener(-1);

        DatabaseReference ref = database.getReference();
        String petitions = hs.get("petitions");

        // check if user is already a member
        /*String[] members = hs.get("users").split(";");
        for(String s: members) {
            if(s.equals(username)) {
                //this.courses_id_stamps += hs.get("id") + ";";
                //this.requestSignaturesInfo(this.courses_id_stamps.split(";"));
                return;
            }
        }*/

        // check if already requested petitions
        if(petitions != null) {
            for(String e: petitions.split(";")) {
                if(e.equals(username)) {
                    Toast.makeText(this, "JOIN REQUEST ALREADY SENT", Toast.LENGTH_LONG).show();
                    return;
                }
            }
        }
        ...
        if(petitions == null) {
            ref.child("signatures").child(this.rubrics_ids.get(selected))
                    .child("petitions").setValue(username + ";");
        }
        else {
            ref.child("signatures").child(this.rubrics_ids.get(selected))
                    .child("petitions").setValue(petitions + username + ";");
        }

        Toast.makeText(this, "JOIN REQUEST SENT", Toast.LENGTH_LONG).show();
        loadActivityView();
    }
            ...
            switch (result_type) {
                ...
                case InformationTracker.SIGNATURE_TRACKER:
                    if(output.getContent().get("special_methods").equals("create")) {
                        ...
                        output.getContent().remove("special_methods");
                        fillNavBar(output.getContent());
                    }
                    break;
                ...
                case InformationTracker.RUBRIC_TRACKER:
                    ...
                    break;
                case InformationTracker.SIGNATURE_TRACKER_DEEP_2:
                    requestPetition(output.getContent());
                    break;
            }
        }
    }
    ...
    public boolean onNavigationItemSelected(MenuItem item) {
        ...
        join_tracker = new InformationTracker(InformationTracker.SIGNATURE_TRACKER_DEEP_2, database, this.rubrics_ids.get(id));
        join_tracker.addListener(this);
